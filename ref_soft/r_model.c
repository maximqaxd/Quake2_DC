...
model_t *Mod_ForName (char *name, qboolean crash)
{
    ...
    // Calculate allocation size based on file type and size
    int allocation_size;
    switch (LittleLong(*(unsigned *)buf))
    {
        case IDALIASHEADER:
        {
            dmdl_t *pinmodel = (dmdl_t *)buf;
            // Get actual model size plus padding for alignment and extra data
            allocation_size = LittleLong(pinmodel->ofs_end) + 16384; // 16KB padding
            if(allocation_size > 0x100000) // Safety cap at 1MB
                allocation_size = 0x100000;
            loadmodel->extradata = Hunk_Begin(allocation_size);
        ...
        case IDSPRITEHEADER:
        {
            // Sprites are typically small, use file size plus small padding
            allocation_size = modfilelen + 4096; // 4KB padding
            if(allocation_size > 0x10000) // Cap at 64KB
        ...
        case IDBSPHEADER:
        {
            // BSP needs more space for processing
            allocation_size = modfilelen * 2; // Double file size for processing
            if(allocation_size > 0x400000) // Cap at 4MB
                allocation_size = 0x400000;
            loadmodel->extradata = Hunk_Begin(allocation_size);
...
//
	for (i=0 ; i<pheader->num_frames ; i++)
	{
		...
		memcpy_fast (poutframe->name, pinframe->name, sizeof(poutframe->name));
		for (j=0 ; j<3 ; j++)
		{
		...
		memcpy_fast (poutframe->verts, pinframe->verts, 
	...
	// register all skins
	memcpy_fast ((char *)pheader + pheader->ofs_skins, (char *)pinmodel + pheader->ofs_skins,
